...
const setUpAutoUpdate = () => {
  if (global.DEV_MODE || true) return;
  try {
    autoUpdater.setFeedURL(`https://update.something.somewhere/update/${platform}/${app.getVersion()}`);
...
const checkUpdateServer = () => {
  https.get('https://update.somewhere.something', () => {
    setUpAutoUpdate();
  }).on('error', () => {
